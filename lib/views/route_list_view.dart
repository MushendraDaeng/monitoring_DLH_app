...
class RouteListView extends StatefulWidget {
  ...
  const RouteListView({required this.routeId, super.key});
...
class _RouteListViewState extends State<RouteListView> {
  ...
  var selectedLocation = const LatLng(1.2666198485659967, 124.88656767402253).obs;
  ...
  void _setMarkers({LatLng customPoint = const LatLng(1.2666198485659967, 124.88656767402253), bool self = false}) async {
    _markers.add(Marker(markerId: const MarkerId("_centerMarker"), position: customPoint, icon: self ? BitmapDescriptor.defaultMarker : BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueOrange), onTap: () {}));
  ...
  void _onMapCreated(GoogleMapController controller) async {
    ...
    Future.delayed(const Duration(seconds: 1), () async {
      ...
      controller.animateCamera(CameraUpdate.newCameraPosition(CameraPosition(target: center, zoom: 16.0)));
    });
  }
  ...
  Future<void> getRouteList() async {
    ...
    var response = await _routeController.getRoute(context: context, id: widget.routeId);
  ...
  Future<void> startTracking() async {
    if (await confirmTracking()) {
      ...
      currentTrackings.value.startLocation = "${cLocation.latitude},${cLocation.longitude}";
      currentTrackings.value.startTime = DateTime.now();
      if (mounted) {
        var response = await _trackingController.startTracking(newData: currentTrackings.value, context: context);
        if (response != null) {
          currentTrackings.refresh();
          showMessage(title: "Success", message: "Tracking has been started", type: MessageType.success);
          if (mounted) {
            Navigator.pop(context);
            Navigator.pop(context, 2);
          }
        } else {
          showMessage(title: "Failed", message: "Tracking failed to start!", type: MessageType.warning);
          if (mounted) {
            Navigator.pop(context);
          }
        }
  ...
  Future<void> stopTracking() async {
    ...
    var cLocation = await location.getLocation();
    currentTrackings.value.stopLocation = "${cLocation.latitude},${cLocation.longitude}";
    currentTrackings.value.stopTime = DateTime.now();
    if (mounted) {
      var response = await _trackingController.stopTracking(newData: currentTrackings.value, context: context);
      if (response != null) {
        currentTrackings.refresh();
        showMessage(title: "Success", message: "Tracking has been stopped", type: MessageType.success);
        if (mounted) {
          Navigator.pop(context);
          Navigator.pop(context, 1);
        }
      } else {
        showMessage(title: "Failed", message: "Tracking failed to stopped!", type: MessageType.warning);
        if (mounted) {
          Navigator.pop(context);
        }
      }
    ...
    return WillPopScope(
        ...
        child: Scaffold(
          appBar: AppBar(
            ...
            title: Column(
              children: [
                Text("Route List", style: style.typography(size: style.textH5, color: style.primaryColor, bold: true)),
                Text(
                    ...
                    children: [
                      Expanded(
                        child: Container(
                          ...
                          decoration: BoxDecoration(color: style.backgroundColor, borderRadius: const BorderRadius.only(bottomLeft: Radius.circular(20), bottomRight: Radius.circular(20))),
                          child: Column(
                            children: [
                              Expanded(
                                child: Container(
                                  ...
                                  child: ClipRRect(
                                    ...
                                    child: GoogleMap(
                                      ...
                                      onCameraMove: (CameraPosition position) async {
                                        ...
                                        selectedLocation.value = _centerMarker.position;
                                      },
                                    ),
                                  ),
                                ),
                              ...
                              Text("Customer List on the Route : ", style: style.typography(size: style.textDesc, bold: true, color: style.darkColor)),
                              const SizedBox(
                                height: 10,
                              ),
                              Expanded(
                                child: Container(
                                  decoration: BoxDecoration(borderRadius: BorderRadius.circular(20), border: Border.all(color: style.darkColor)),
                                  child: ListView.builder(
                                      itemCount: routeDetails.isEmpty ? 1 : routeDetails.length,
                                      itemBuilder: (context, index) {
                                        if (routeDetails.isEmpty) {
                                          return Container(
                                            ...
                                            height: 100,
                                            alignment: Alignment.center,
                                            decoration: BoxDecoration(color: style.accentColor.withOpacity(0.4), borderRadius: BorderRadius.circular(10)),
                                            child: Text("No Data Customers yet!", style: style.typography(size: style.textH6, bold: true, color: style.darkColor)),
                                          );
                                        }
                                        return InkWell(
                                          onTap: () {
                                            var pos = LatLng(routeDetails[index].customerLat, routeDetails[index].customerLng);
                                            _gMapController.animateCamera(CameraUpdate.newLatLng(pos));
                                            _centerMarker = _centerMarker.copyWith(positionParam: pos);
                                            setState(() {});
                                          },
                                          child: Container(
                                            width: screen.width,
                                            ...
                                            padding: const EdgeInsets.all(10),
                                            decoration: BoxDecoration(border: Border.all(color: style.darkColor), borderRadius: BorderRadius.circular(10), color: style.lightColor),
                                            child: Column(
                                              crossAxisAlignment: CrossAxisAlignment.start,
                                              mainAxisAlignment: MainAxisAlignment.center,
                                              children: [
                                                Text(routeDetails[index].customerName, style: style.typography(size: style.textH5, color: style.darkColor, bold: true)),
                                                Divider(
                                                  color: style.darkColor,
                                                ),
                                                Text("District : ${routeDetails[index].customerVillage}", style: style.typography(size: style.textDesc, color: style.fourthColor)),
                                                Text("Sub District : ${routeDetails[index].customerDistrict}", style: style.typography(size: style.textDesc, color: style.darkColor)),
                                                Text("Status : ${routeDetails[index].customerStatus}", style: style.typography(size: style.textDesc, color: style.darkColor, bold: true))
                                              ],
                                            ),
                                          ),
                                        );
                          ...
                          children: [
                            ...
                            Expanded(
                              child: ElevatedButton(
                                  ...
                                  child: Container(
                                    alignment: Alignment.center,
                                    child: Text(currentTrackings.value.startLocation != '-' ? "Stop Geofencing" : "Start Geofencing", style: style.typography(size: style.textH6, useDefaultColor: true, bold: true)),
                                  )),
                            )
                          ],
                        ),
